...
/-! # llm-d-async sql transport, partition leases

Model of `producer-sql/sqlqueue` at llm-d/llm-d-async#452 head `4ed4210`.
...
-- `Store.Retry` or `Store.Undispatch`: back to pending, fenced on stamp and owner.
...
-- TestConsumerRetryBookkeepingKeepsTheAttemptPollHandedOut
sat trace [retry_then_poll_strands] {
  acquire
  dispatch
  book
  requeueCommit
  ...
  assert (∃ n k e, stranded n k e)
}

sat trace [retry_then_reconcile_strands] {
  acquire
  dispatch
  book
  requeueCommit
